...
export const initialMatrices: Matrix[] = [
  ...
  { id: 'prod', name: 'Northstar Product', description: 'Product craft for PMs driving discovery through execution.',
    familyId: 'prod', version: 1, status: 'Active', scaleSize: 5, owners: ['Lan Nguyen'], changes: 0, editedBy: 'Lan Nguyen', editedAt: '3 days ago',
    history: [{ who: 'Lan Nguyen', what: 'Published', when: '22 Sep 2026, 10:00' }, { who: 'Lan Nguyen', what: 'Created matrix', when: '18 Sep 2026, 09:00' }],
    competencies: [
      { id: 'disc', name: 'Discovery', description: 'Finds and validates the right problem to solve.', behaviors: behaviorsFor('discovery') },
      { id: 'strat', name: 'Product strategy', description: 'Connects product bets to company strategy.', behaviors: behaviorsFor('product strategy') },
      { id: 'exec', name: 'Execution', description: 'Drives a product from idea to shipped outcome.', behaviors: behaviorsFor('execution') },
      { id: 'data', name: 'Data literacy', description: 'Uses data to make and defend product decisions.', behaviors: behaviorsFor('data literacy') },
      { id: 'stake', name: 'Stakeholder management', description: 'Aligns stakeholders around a shared product direction.', behaviors: behaviorsFor('stakeholder management') },
    ] },
...
export const initialPositions: Position[] = [
  { id: 'be', changes: 0, name: 'Backend Engineer', code: 'BE', status: 'Published', matrixId: 'eng', department: 'Engineering', levels: be,
    expectations: grid('eng', be, [[1, 2, 3, 4], [2, 3, 4, 4], [1, 2, 3, null], [2, 3, 3, 4], [1, 2, 3, 5], [null, 1, 3, 4]]),
    editedBy: 'Lan Nguyen', editedAt: '10 min ago',
    history: [{ who: 'Lan Nguyen', what: 'Published', when: 'Today, 09:50' }, { who: 'Lan Nguyen', what: 'Edited 3 expectations', when: 'Today, 09:42' }, { who: 'Minh Tran', what: 'Created position', when: '18 Sep 2026, 14:05' }] },
  ...
  { id: 'pm', changes: 0, name: 'Product Manager', code: 'PM', status: 'Published', matrixId: 'prod', department: 'Product', levels: pm,
    expectations: grid('prod', pm, [[2, 3, null], [1, 2, null], [2, 3, 4], [null, null, null], [1, 3, 4]]),
    editedBy: 'Lan Nguyen', editedAt: 'yesterday',
    history: [{ who: 'Lan Nguyen', what: 'Published', when: '23 Sep 2026, 11:10' }, { who: 'Lan Nguyen', what: 'Created position', when: '21 Sep 2026, 15:30' }] },
